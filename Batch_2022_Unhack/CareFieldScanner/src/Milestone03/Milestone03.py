...
def main() -> None:
    ...
    SFS = [md.sfSize for md in metadata].sort()
    ...
    for cf in careFields:
        ...
        if not covering_id:
            ...
            logger.info(
                f"Created MainField {mcnt} covering bounds ({new_mf.xMin}, {new_mf.yMin}, {new_mf.xMax}, {new_mf.yMax})")
        ...
        while curry <= cf.yMax:
            ...
            while currx <= cf.xMax:
                for i in SFS:
                    if currx + i >= cf.xMax:
                        subFields.append(SubField(scnt, currx, currx + i, curry, curry + i, covering_id))
