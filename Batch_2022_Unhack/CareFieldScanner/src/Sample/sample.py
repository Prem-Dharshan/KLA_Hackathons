...
def main() -> None:
    # Input file paths
    cf_file = "../../MilestoneInput/Sample/Input/CareAreas.csv"
    md_file = "../../MilestoneInput/Sample/Input/metadata.csv"
    ...
    for cf in careFields:
        ...
        # Generate SubFields for the current CareField
        sfsyMin = cf.yMin
        while sfsyMin < cf.yMax:
            sfsyMax = sfsyMin + SFS  # Ensure no overlap, fit within boundaries
            sfsxMin = cf.xMin
            while sfsxMin < cf.xMax:
                sfsxMax = sfsxMin + SFS  # Ensure no overlap, fit within boundaries
                scnt += 1.0
                subFields.append(
                    SubField(scnt, sfsxMin, sfsxMax, sfsyMin, sfsyMax, mcnt)
                )
                sfsxMin += SFS  # Move to the next SubField horizontally
            sfsyMin += SFS  # Move to the next SubField vertically

    output_file = "../../MilestoneOutput/Sample/mainfields.csv"
    os.makedirs(os.path.dirname(output_file), exist_ok=True)

    s_output_file = "../../MilestoneOutput/Sample/subfields.csv"
    os.makedirs(os.path.dirname(output_file), exist_ok=True)
